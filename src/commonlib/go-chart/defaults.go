...
import (
	...
	"commonlib/freetype/truetype"
	"commonlib/go-chart/drawing"
	"global"
)
...
var (
	...
	ColorBlackTransparent           = drawing.Color{R: 192, G: 192, B: 192, A: 64}
	ColorBlackTransparentBackground = drawing.Color{R: 241, G: 241, B: 241, A: 64}
...
func GetDefaultFont() (*truetype.Font, error) {
	if _defaultFont == nil {
		...
		if _defaultFont == nil {
			heiti, _ := ioutil.ReadFile(global.ConfigMappings[global.FONT_HEITI_PATH]) //黑体
			font, err := truetype.Parse(heiti)
			//font, err := truetype.Parse(roboto) //roboto 这个字体不支持中文，坑
			if err != nil {
				return nil, err
			}
			_defaultFont = font
